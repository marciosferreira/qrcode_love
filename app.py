...
mp = mercadopago.SDK("APP_USR-6523172338846242-100514-afc9490970db73cf999cadf640593b3e-60830907")     #production
...
def login():
    if request.method == 'POST':
        ...
        if username == 'marcioverde' and check_password_hash(generate_password_hash('cpfl2002'), password):
            user = User(1)
            login_user(user)
            return redirect(url_for('list_dynamo_items'))  # Redireciona para a página de itens após login
        else:
            flash('Login ou senha incorretos.')
            return redirect(url_for('login'))  # Se falhar, redireciona de volta para a página de login
...
def edit_user(email, page_url):
    try:
        ...
        if request.method == 'POST':
            ...
            user['created_at'] = f'{new_date}T{new_time}:00.000Z'  # Formato completo de timestamp

            # Captura o valor da checkbox (se marcada, é 'on', caso contrário não está presente)
            user['paid'] = 'paid' in request.form  # retorna True se a checkbox estiver marcada, senão False
...
def send_email_with_qr_attachment(to_address, subject, body, qr_image_bytes, filename="qrcode.png"):
    client = boto3.client('ses', region_name='us-east-1')
    print("qr image bytes")
    print(qr_image_bytes)
    # Cria a mensagem MIME
    msg = MIMEMultipart()
    msg['Subject'] = subject
    msg['From'] = 'contato@qrcodelove.me'  # Substitua pelo seu e-mail
    msg['To'] = to_address
...
def delete_old_pages():
    try:
        ...
        for item in response['Items']:
            ...
            if created_at:
                try:
                    ...
                    created_at_date = created_at_date.replace(tzinfo=pytz.utc).astimezone(timezone)
                except ValueError:
        ...
        for page in old_pages:
            primary_key = {
                'email': page['email'],  # Ajuste conforme o nome da chave primária da sua tabela
            ...
            try:
                objects_to_delete = s3_client.list_objects_v2(Bucket=S3_BUCKET, Prefix=folder_key)
...
def delete_item(email, page_url):
    ...
    try:
        ...
        print(f'Erro ao deletar o folder {folder_key} do S3: {e}')
        flash(f'Erro ao deletar o folder {folder_key} no S3. Por favor, tente novamente.', 'error')
    ...
    try:
        ...
        print(f'Erro ao deletar o QRCode {qrcode_key} do S3: {e}')
        flash(f'Erro ao deletar o QRCode {qrcode_key} no S3. Por favor, tente novamente.', 'error')
...
def deletar_pagina():
    if request.method == 'GET':
        ...
        if items:
            ...
            flash('Página não encontrada. Verifique o email e o código informados.', 'error')
            return redirect(url_for('deletar_pagina'))
    ...
    try:
        ...
        for i, image in enumerate(images):
            if image and allowed_file(image.filename):
                filename = secure_filename(f'{i+1}.png')  # Renomeando as imagens para 1.png, 2.png, 3.png
                s3_key = f'pictures/{unique_code}/{filename}'
                try:
                    #redimensiona se muito grande
                    resized_image = resize_image_if_needed(image)
                    # Enviar imagem ao S3
                    s3_client.upload_fileobj(resized_image, S3_BUCKET, s3_key)  # Torna o arquivo publicamente acessível
                except Exception as e:
                    return str(e), 500
            else:
        ...
        new_couple = {
            ...
            'email': email,
            'paid': False,
            'created_at': datetime.now(timezone).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
            'video_id': video_id
        }
        ...
        response = send_email_with_qr_attachment(email, email_subject, email_body, qr_image_bytes, filename="qrcode.png")
        print("response")
        print(response)
...
def couple_page(page_url):
    ...
    event_date_str = couple['event_date']  # Pega a string da data do evento (ex: '2024-10-19')
    event_time_str = couple.get('event_time', '00:00')  # Pega a hora, ou usa '00:00' se não existir
    ...
    response = s3_client.list_objects_v2(Bucket='qrcodelove-pictures', Prefix=f'pictures/{page_url}/')
    print(response)
    ...
    return render_template('couple_page.html',
                           has_video=has_video,
                           couple=couple,
...
def webhook():
    ...
    if notification_data.get('type') == 'payment':
        ...
        if payment_info['response']['status'] == 'approved':
            ...
            couple['created_at'] = datetime.now(timezone).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z'
...
def generate_qr_code(url, unique_code, box_size=20, border=4):
    try:
        ...
        image_bytes.seek(0)  # Certifique-se de que o ponteiro do BytesIO esteja no início

        # Definir a chave (nome do arquivo) para o S3
        s3_key = f'qrcodes/{unique_code}.png'

        # Criar um novo BytesIO para o upload
        image_bytes_for_s3 = io.BytesIO(image_bytes.getvalue())  # Cria um novo BytesIO para o upload
        image_bytes_for_s3.seek(0)  # Certifique-se de que o ponteiro do BytesIO esteja no início
